...
function openGraphHelper(options = {}) {
  ...
  const twitterCard = options.twitter_card || 'summary_large_image';
  ...
  if (date) {
    if ((isMoment(date) || isDate(date)) && !isNaN(date.valueOf())) {
      ...
      result += meta('weibo:article:create_at', date.toISOString());
    }
  }
  ...
  if (updated) {
    if ((isMoment(updated) || isDate(updated)) && !isNaN(updated.valueOf())) {
      ...
      result += meta('weibo:article:update_at', date.toISOString());
    }
  }
  ...
  let image = config.favicon; // init image as favicon
  if(this.is_post()){
    if(page.thumbnail){
  ...
  result += og('og:image', image, false);
  result += meta('twitter:image', image, false);

// I dont't need multiple og:image to select at this time. https://stackoverflow.com/questions/13424780/facebook-multiple-ogimage-tags-which-is-default
